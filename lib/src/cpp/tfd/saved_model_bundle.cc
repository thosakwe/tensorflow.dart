...
void tfd::SavedModelBundle_new(Dart_NativeArguments arguments) {
  ...
  HandleError(Dart_StringToCString(Dart_GetNativeArgument(arguments, 0),
                                   &exportDirectory)); // String exportDirectory

  Dart_Handle tagsHandle =
      Dart_GetNativeArgument(arguments, 1); // List<String> tags
  intptr_t length;
  ...
  if (!Dart_IsNull(runOptionsHandle)) {
    ...
    intptr_t len;
    HandleError(
        Dart_TypedDataAcquireData(runOptionsHandle, &type, &data, &len));
  ...
  TF_Session *session = TF_LoadSessionFromSavedModel(
      opts, runOptions, exportDirectory, tags, nTags, graph, metagraph, status);
  ...
  if (code != 0) {
    ...
    tuple[3] = Dart_NewExternalTypedData(
        Dart_TypedData_kUint8, (void *)metagraph->data, metagraph->length);
  }

...
TF_Tensor *tfd::convert_tensor(Dart_Handle handle) {
    Dart_Handle dtypeHandle = Dart_GetField(handle, Dart_NewStringFromCString("_dataType")); // int
    Dart_Handle shapeHandle = Dart_GetField(handle, Dart_NewStringFromCString("_shape")); // Int64List
    Dart_Handle dataHandle = Dart_GetField(handle, Dart_NewStringFromCString("_data")); // Uint8List
    ...
    HandleError(Dart_TypedDataAcquireData(dataHandle, &type, (void **) &data, &length));
    if (dtype == TF_STRING) length = 8 + TF_StringEncodedSize(strlen(data));
    auto *tensor = TF_AllocateTensor((TF_DataType) dtype, dims, (int) nDims, (size_t) length);
    ...
    if (tensor == nullptr) {
        ...
        if (TF_TensorType(tensor) == TF_STRING) {
            ...
            TF_StringEncode(data, strlen(data), 8 + (char *) tensorData, TF_StringEncodedSize(strlen(data)), status);
        } else {
...
Dart_Handle tfd::get_tensor_value(TF_Tensor *tensor) {
    if (tensor == nullptr) return Dart_Null();
...
TF_OperationDescription *tfd::dereference_operation_description_ptr(Dart_Handle handle) {

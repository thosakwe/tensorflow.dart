...
void tfd::Tensors_string(Dart_NativeArguments arguments) {
  ...
  auto *tensor = TF_AllocateTensor(TF_STRING, nullptr, 0,
                                   8 + TF_StringEncodedSize(strlen(str)));

  if (tensor == nullptr)
    throwCoreError("Could not allocate string tensor.", "StateError");
  else {
    auto *data = TF_TensorData(tensor);
    memset(data, 0, TF_TensorByteSize(tensor));
    TF_StringEncode(str, strlen(str), 8 + (char *)data,
                    TF_StringEncodedSize(strlen(str)), status);
    ...
    if (code != 0) {
      ...
      // Get the raw data.
      Dart_Handle dataHandle = tuple[2] =
          Dart_NewTypedData(Dart_TypedData_kUint8, TF_TensorByteSize(tensor));
      void *buf;
      intptr_t length;
    ...
    Dart_Handle tupleHandle = DartListOf(tuple, 3);
...
void tfd::Constant(Dart_NativeArguments arguments) {
  ...
  HandleError(Dart_StringToCString(Dart_GetNativeArgument(arguments, 3),
                                   &operationName));
  HandleError(
      Dart_IntegerToInt64(Dart_GetNativeArgument(arguments, 4), &dtype));

  // Make a Output._()
  Dart_Handle outputInstance =
      Dart_New(outputType, Dart_NewStringFromCString("_"), 1, &graphHandle);
  ...
  if (Dart_IsString(value)) {
    const char *str;
    HandleError(Dart_StringToCString(value, &str));
    tensor = TF_AllocateTensor(TF_STRING, nullptr, 0,
                               8 + TF_StringEncodedSize(strlen(str)));
    TF_StringEncode(str, strlen(str), 8 + (char *)TF_TensorData(tensor),
                    TF_StringEncodedSize(strlen(str)), status);
    ...
    *((float *)(TF_TensorData(tensor))) = (float)v;
    ...
    if (length == 0) {
      ...
      if (dtype == TF_INT8) {
        tensor = TF_AllocateTensor((TF_DataType)dtype, &dims, nDims,
                                   sizeof(int8_t) * length);
        ...
        for (intptr_t i = 0; i < length; i++) {
          HandleError(
              Dart_IntegerToUint64(Dart_ListGetAt(value, i), &(arr[i])));
        }
        ...
        for (intptr_t i = 0; i < length; i++) {
          ...
          memset(ptr, 0, 8);
          ptr += 8;
          TF_StringEncode(str, strlen(str), (char *)ptr,
                          TF_StringEncodedSize(strlen(str)), status);
          ptr += size;
        }
      }
  ...
  if (op == nullptr) {
    Dart_ThrowException(
        Dart_NewStringFromCString("Failed to add node to graph."));
  }

  HandleError(Dart_SetField(outputInstance,
                            Dart_NewStringFromCString("_operation"),
                            Dart_NewIntegerFromUint64((uint64_t)op)));
  HandleError(Dart_SetField(outputInstance, Dart_NewStringFromCString("_index"),
                            Dart_GetNativeArgument(arguments, 5)));
  HandleError(Dart_SetField(outputInstance, Dart_NewStringFromCString("_dtype"),
                            Dart_NewInteger(TF_TensorType(tensor))));

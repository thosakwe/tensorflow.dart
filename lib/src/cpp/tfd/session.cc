...
void tfd::SessionRunGraph(Dart_NativeArguments arguments) {
  ...
  Dart_Handle operationHandle = HandleError(
      Dart_GetField(outputInstance, Dart_NewStringFromCString("_operation")));
  Dart_Handle indexHandle = HandleError(
      Dart_GetField(outputInstance, Dart_NewStringFromCString("_index")));
  ...
  // Return the tuple.
  auto tupleInstance = DartListOf(tuple, 3);
...
void tfd::Session_run(Dart_NativeArguments arguments) {
  ...
  Dart_Handle configHandle =
      Dart_GetNativeArgument(arguments, 1); // Uint8List config
  Dart_Handle runOptionsHandle =
      Dart_GetNativeArgument(arguments, 2); // Uint8List runOptions
  Dart_Handle inputTensorsHandle =
      Dart_GetNativeArgument(arguments, 3); // List<Uint8List> inputTensors
  Dart_Handle outputsHandle =
      Dart_GetNativeArgument(arguments, 4); // List<Output> outputs
  Dart_Handle nOutputsHandle =
      Dart_GetNativeArgument(arguments, 5); // int nOutputs
  Dart_Handle targetsHandle =
      Dart_GetNativeArgument(arguments, 6); // List<int> targets
  Dart_Handle inputsHandle =
      Dart_GetNativeArgument(arguments, 7); // List<Output> inputs
  ...
  if (!Dart_IsNull(outputsHandle) && nOutputs > 0) {
    outputs = (struct TF_Output *)Dart_ScopeAllocate(sizeof(struct TF_Output) *
                                                     nOutputs);
  ...
  if (nOutputs > 0)
    tensorOutput =
        (TF_Tensor **)Dart_ScopeAllocate(sizeof(struct TF_Tensor *) * nOutputs);
  ...
  if (length > 0) {
    nTargets = (int)length;
    targets =
        (TF_Operation **)Dart_ScopeAllocate(sizeof(TF_Operation *) * length);
  ...
  if (nInputs > 0) {
    inputTensors =
        (TF_Tensor **)Dart_ScopeAllocate(sizeof(TF_Tensor *) * nInputs);

    for (intptr_t i = 0; i < nInputs; i++) {
      // Convert a TensorProto to a Tensor, on-the-fly.
      Dart_Handle tensorHandle =
          Dart_ListGetAt(inputTensorsHandle, i); // TensorProto
  ...
  if (tensorOutput != nullptr) {
    for (int i = 0; i < nOutputs; i++) {
      ...
      // Dart_ListSetAt(tensors, i,
      // Dart_NewExternalTypedData(Dart_TypedData_kUint8, data, size));
    }
  }

  // Create a Uint8List for the run metadata.
  tuple[3] = Dart_NewExternalTypedData(
      Dart_TypedData_kUint8, (void *)metadata->data, metadata->length);

  // Create a tuple.
  auto out = DartListOf(tuple, 4);
